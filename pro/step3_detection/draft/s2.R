...
FOLDER_good = dirname(dirname(FOLDER_position))
file = "DATA_the_book_df.rds"
pathfile = paste0(FOLDER_good,"/step2_structuration/data/",file)
df = readRDS(file = pathfile)
df$url = paste0("https://gallica.bnf.fr/ark:/12148/bpt6k96718891/f",df$page_id_pdf,".image") # rajouter url
# df$filter = df$paragraph_id == 1
...
FUN_build_htmlstring_content = function(df,apattern,string_command){
  ...
  df$HTML_object[!df$filter] = FUN_format_color(df$paragraph_value[!df$filter])
  ...
  df$HTML_meta = FUN_format_color(meta_vector)
  ...
  solution = paste0(df$HTML_object,collapse = " ")
...
# r = FUN_build_htmlstring_content(df,
#                                  apattern="^[A-Z]",
#                                  string_command ="!is.na(df[,1])")
...
# format metadata
FUN_format_color=function(text_vector){
  res = paste0("<span style='color:grey'>",text_vector,"</span>")
  return(res)
}
